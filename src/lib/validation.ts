 ...
 *
 * 設計方針:
 * - 過度に厳格な制約を避け、実用的な柔軟性を確保
 * - 最終的な検証はメール送信時に行われる
 * - セキュリティリスクのある文字（<, >, スペースなど）は除外
 *
 * ローカル部（@の前）:
 * - 英数字と一般的な記号（. _ - + ! # $ % & ' * / = ? ^ ` { | } ~）を許可
 * - 連続するドットは禁止
 ...
 * セキュリティ上の注意:
 * ローカル部で許可している一部の記号（! # $ % & ' * / = ? ^ { | } ~）は、
 * RFC 5322では有効ですが、以下のリスクがあります：
 * - {, }, |, ^: システムによってはコマンドインジェクションのリスク
 * - !, %: URLエンコーディング時の問題
 * - /: パス区切り文字として誤認される可能性
 * 厳格な制限が必要な場合は、正規表現を [a-zA-Z0-9._+-] のみに限定してください。
 */
