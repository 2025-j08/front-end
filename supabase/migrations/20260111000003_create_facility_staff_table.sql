...
CREATE TABLE public.facility_staff (
    ...
    -- データの妥当性チェック
    CONSTRAINT chk_staff_data_not_empty CHECK (jsonb_typeof(data) = 'object' AND data != '{}'::jsonb)
);
...
CREATE INDEX idx_facility_staff_data_gin ON public.facility_staff USING gin(data);
...
COMMENT ON COLUMN public.facility_staff.data IS '職員情報を含む JSONB データ';
