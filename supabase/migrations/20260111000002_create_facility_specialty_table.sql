...
CREATE TABLE public.facility_specialty (
    ...
    -- データの妥当性チェック
    CONSTRAINT chk_specialty_data_not_empty CHECK (jsonb_typeof(data) = 'object' AND data != '{}'::jsonb)
);
...
CREATE INDEX idx_facility_specialty_data_gin ON public.facility_specialty USING gin(data);
...
COMMENT ON COLUMN public.facility_specialty.data IS '生活環境・特色情報を含む JSONB データ';
