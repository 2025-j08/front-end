...
CREATE TABLE public.facility_philosophy (
    ...
    -- データの妥当性チェック
    CONSTRAINT chk_philosophy_data_not_empty CHECK (jsonb_typeof(data) = 'object' AND data != '{}'::jsonb)
);
...
CREATE INDEX idx_facility_philosophy_data_gin ON public.facility_philosophy USING gin(data);
...
COMMENT ON COLUMN public.facility_philosophy.data IS '理念情報を含む JSONB データ';
